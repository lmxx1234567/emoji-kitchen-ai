...
def train(checkpoint_path_gen=None,checkpoint_path_dis=None,frezze_feature_extractor=False):
    ...
    optimizer_dis = optim.Adam(discriminator.parameters(), lr=0.000001, betas=(0.5, 0.999))
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i,(left_img, right_img, real_merged_img) in enumerate(dataloader):  # Assume dataloader is created from your EmojiDataset
            ...
            loss_real = criterion(logits_real, real_labels*0.9)
            loss_fake = criterion(logits_fake, fake_labels+0.1)
            ...
            loss_gen = criterion(logits_fake, real_labels*0.9)  # We want the generator to fool the discriminator
            ...
            running_acc += torch.sum((logits_fake > 0.5) == real_labels).item()
            if i % print_every == 0:
                running_loss_dis = 0.0
                running_loss_gen = 0.0
                running_acc = 0.0
            else:
                pbar.set_postfix(loss_gen=running_loss_gen / (i%print_every),loss_dis=running_loss_dis / (i%print_every), acc=running_acc/(i%print_every*batch_size))
            pbar.update(1)
